...
public class SystemBarHelper {
     ...
     * @param alpha          透明栏透明度[0.0-1.0]
     */
    public static void tintStatusBar2(Window window, @ColorInt int statusBarColor, @FloatRange(from = 0.0, to = 1.0) float alpha) {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.KITKAT) {
            return;
        }
    ...
    /**
     * Android4.4以上, 让Toolbar 透明并且覆盖到系统状态栏上面
     *
     * @param window         一般都是用于Activity的window,也可以是其他的例如Dialog,DialogFragment
     * @param statusBarColor 状态栏颜色
     * @param alpha          透明栏透明度[0.0-1.0]
     */
    public static void tintStatusBarWithToolBar(Window window, @ColorInt int statusBarColor, @FloatRange(from = 0.0, to = 1.0) float alpha) {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.KITKAT) {
            return;
        }
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            window.clearFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS);
            window.addFlags(WindowManager.LayoutParams.FLAG_LAYOUT_NO_LIMITS);//设置允许该应用程序的窗口可以扩散到屏幕外围。
            window.setStatusBarColor(Color.TRANSPARENT);
        } else {
            window.addFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS);
        }
    ...
    public static void immersiveStatusBar(Window window, @FloatRange(from = 0.0, to = 1.0) float alpha, boolean needFitsWindow) {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.KITKAT) {
            return;
        }
     ...
     * 增加View的高度以及paddingTop,增加的值为状态栏高度.一般是在沉浸式全屏给ToolBar用的
     */
    public static void adjustHeightAndPadding(Context context, View view, int oldStatusBarHeight) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            int add = getStatusBarHeight(context) - oldStatusBarHeight;
            ViewGroup.LayoutParams lp = view.getLayoutParams();
            lp.height += add;//增高
            view.setPadding(view.getPaddingLeft(), view.getPaddingTop() + add,
                    view.getPaddingRight(), view.getPaddingBottom());
        }
    }
    ...
    /**
     * 增加View的MarginTop,增加的值为状态栏高度
     */
    public static void setMarginsToStatusBarHeight(Context context, View v) {
        int statusBarHeight = SystemBarHelper.getStatusBarHeight(context);
        statusBarHeight = statusBarHeight == 0 ? DensityUtils.dp2px(context, 20) : statusBarHeight;
        setMargins(v, 0, statusBarHeight, 0, 0);
    }


    /**
     * 设置react native页面的状态栏
     */
    public static void setReactnativeStatusBar(Window window) {
